...
export default function Navbar() {
  ...
  useEffect(() => {
    if (document.getElementById('trfc-nav-styles')) return
  ...
  return (
    <>
      ...
      <nav className="trfc-nav">
        <div className="trfc-nav__topline" />

        <div className={`trfc-nav__bar${scrolled ? ' scrolled' : ''}`}>
          <div className="trfc-nav__inner">

            {/* Logo */}
            <Link to="/" className="trfc-nav__logo">
              <span className="trfc-nav__logo-main">TR<span>F</span>C</span>
              <span className="trfc-nav__logo-sub">Thika Road FC</span>
            </Link>

            {/* Desktop nav links */}
            <div className="trfc-nav__links">
              {navLinks.map((l) => (
                <Link
                  key={l.to}
                  to={l.to}
                  className={`trfc-nav__link${isActive(l.to) ? ' active' : ''}`}
                >
                  {l.label}
                </Link>
              ))}
            ...
            {/* Desktop actions */}
            <div className="trfc-nav__actions">
              <ThemeToggle />
              <Link to="/cart" className="trfc-nav__cart" aria-label={`Cart (${cartCount} items)`}>
                <ShoppingCart size={18} />
                {cartCount > 0 && (
                  <span className="trfc-nav__cart-badge">{cartCount > 9 ? '9+' : cartCount}</span>
                )}
              </Link>

              {token ? (
                <button onClick={handleLogout} className="trfc-nav__btn-ghost">
                  <LogOut size={13} />
                  Logout
                </button>
              ) : (
                <>
                  <Link to="/login" className="trfc-nav__btn-ghost">
                    <LogIn size={13} />
                    Login
                  </Link>
                  <Link to="/register" className="trfc-nav__btn-primary">
                    <UserPlus size={13} />
                    Join
                  </Link>
                </>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Link to="/cart" className="trfc-nav__cart-mobile" aria-label="Cart">
                <ShoppingCart size={18} />
                {cartCount > 0 && (
                  <span className="trfc-nav__cart-badge">{cartCount > 9 ? '9+' : cartCount}</span>
                )}
      ...
      {/* Mobile Drawer */}
      <div className={`trfc-nav__drawer${isOpen ? ' open' : ''}`} aria-hidden={!isOpen}>
        <div className="trfc-nav__drawer-overlay" onClick={() => setIsOpen(false)} />
        <div className="trfc-nav__drawer-panel" role="dialog" aria-modal="true">

          <div className="trfc-nav__drawer-head">
            <Link to="/" className="trfc-nav__logo" onClick={() => setIsOpen(false)}>
              <span className="trfc-nav__logo-main" style={{ fontSize: '28px' }}>
                TR<span style={{ color: 'var(--fire)' }}>F</span>C
              </span>
              <span className="trfc-nav__logo-sub">Thika Road FC</span>
            </Link>
            <button className="trfc-nav__drawer-close" onClick={() => setIsOpen(false)} aria-label="Close menu">
              <X size={16} />
            </button>
          </div>

          <div className="trfc-nav__drawer-links">
            {navLinks.map((l) => (
              <Link
                key={l.to}
                to={l.to}
                className={`trfc-nav__drawer-link${isActive(l.to) ? ' active' : ''}`}
          ...
          <div className="trfc-nav__drawer-footer">
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '10px' }}>
              <ThemeToggle />
            </div>
            {token ? (
              <button onClick={handleLogout} className="trfc-nav__btn-ghost" style={{ width: '100%', justifyContent: 'center', padding: '14px' }}>
                <LogOut size={15} />
                Logout
              </button>
            ) : (
              <>
                ...
                <Link
                  to="/register"
                  className="trfc-nav__btn-primary"
                  style={{ justifyContent: 'center', padding: '14px', clipPath: 'polygon(0 0, calc(100% - 10px) 0, 100% 10px, 100% 100%, 10px 100%, 0 calc(100% - 10px))' }}
                  onClick={() => setIsOpen(false)}
                >
                  <UserPlus size={15} />
                  Join the Community
