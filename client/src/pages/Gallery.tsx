    ...
    <div className={pageRoot}>
        ...
        {loading && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0.5">
            {Array(9).fill(null).map((_, i) => (
              <div
                key={i}
                className={`bg-ash light:bg-ash-light relative overflow-hidden ${i === 0 ? 'md:col-span-2 md:row-span-2' : ''}`}
                style={{ aspectRatio: '1' }}
              >
                ...
                >
                  ...
                  {item.caption && (
                    <div className="absolute bottom-0 left-0 right-0 px-4 pb-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-y-0.5 group-hover:translate-y-0">
                      <p className="text-xs text-chalk light:text-chalk-light italic leading-tight line-clamp-2">{item.caption}</p>
                    </div>
                  )}
      ...
      {selectedItem && (
        ...
        >
          {/* Panel */}
          <div
            className={`relative w-full max-w-4xl flex flex-col bg-ink light:bg-ink-light border border-white/10 light:border-black/10 overflow-hidden clip-angled`}
          ...
          >
            ...
            <div className="relative bg-night light:bg-night-light flex items-center justify-center" style={{ minHeight: '420px', maxHeight: '65vh' }}>
              ...
              <GalleryMedia
                ...
                item={selectedItem}
                className={`max-w-full object-contain transition-opacity duration-300 ${imgLoaded || isVideo(selectedItem) ? 'opacity-100' : 'opacity-0'}`}
            ...
            {(selectedItem.caption || selectedItem.uploaded_at) && (
              <div className="px-6 py-4 border-t border-white/5 light:border-black/8 flex items-start justify-between gap-4 flex-wrap">
                {selectedItem.caption && (
                  <p className="text-sm text-chalk/60 light:text-chalk-light/60 italic leading-relaxed flex-1">
                    {selectedItem.caption}
                  </p>
                )}
            ...
            {/* Thumbnail strip */}
            {items.length > 1 && (
              <div className="border-t border-white/5 light:border-black/8 px-4 py-3 flex gap-1.5 overflow-x-auto">
                {items.map((thumb, i) => (
                  <button
                    key={thumb.id}
                    onClick={() => openItem(i)}
                  ...
                  >
                    <GalleryMedia item={thumb} thumbnail className="w-full h-full object-cover" />
                  </button>
                ))}
              </div>
            )}
