...
export default function Home() {
  ...
  return (
    <div className={pageRoot}>
      ...
      <section className="relative min-h-screen w-full flex items-center overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto px-[6%] py-[80px] relative z-10 w-full">
          ...
          <div className="hero-ctas flex flex-wrap gap-4 mb-15">
            <Link to="/register" className="font-barlow-condensed font-black text-base tracking-wider text-black light:text-white px-9 py-3.5 bg-accent light:bg-accent-light clip-angled-lg transition-all duration-200 hover:bg-accent/90 light:hover:bg-accent-light/90 hover:scale-104 inline-block">
              Join the Community
            </Link>
      ...
      {/* ── TICKER ─────────────────────────────────────────────────── */}
      <div className="bg-accent light:bg-accent-light overflow-hidden py-3">
        <style>{`
          .ticker-inner {
            display: flex;
            white-space: nowrap;
          ...
          <div>
            {[
              ...
              <Link to={f.to} key={f.n} className="flex items-center gap-3 py-4.5 border-b border-white/7 light:border-black/7 no-underline transition-all duration-200 hover:gap-4.5 group" style={{ color: 'inherit' }}>
                ...
                <span className="text-smoke light:text-mist transition-colors duration-200 group-hover:text-accent light:hover:text-accent-light">→</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-[6%] bg-ink light:bg-ink-light border-t border-white/5 light:border-black/5">
        <div className="max-w-[1200px] mx-auto grid grid-cols-4 gap-px">
          {[
            ...
            <div
              key={s.label}
              className={`stat-card py-10 px-7 text-center ${i % 2 === 0 ? 'bg-ash light:bg-ash-light' : 'bg-smoke light:bg-smoke-light'} ${i === 0 ? 'border-l-3 border-accent light:border-accent-light' : ''}`}
              style={{ '--delay': `${i * 0.12}s` } as any}
            >
              <p className="font-bebas text-[clamp(48px,5vw,72px)] text-accent light:text-accent-light leading-none mb-1.5">{s.val}</p>
      ...
      {/* ── CTA BAND ──────────────────────────────────────────────── */}
      <section className="py-20 px-[6%] bg-ink light:bg-ink-light border-y border-white/10 light:border-black/10 relative overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto flex items-center justify-between gap-8 flex-wrap relative">
          ...
          <Link
            to="/register"
            className="bg-accent light:bg-accent-light text-black light:text-white px-12 py-4 font-barlow-condensed font-black text-lg tracking-widest uppercase no-underline clip-angled-lg inline-block transition-all duration-200 hover:scale-106 hover:bg-accent/90 light:hover:bg-accent-light/90 animate-pulse-ring"
          >
            Join Now — It's Free
          </Link>
        </div>
        ...
        <section className="py-24 px-[6%] bg-night light:bg-night-light">
          <div className="max-w-[1200px] mx-auto">
            <div className="flex justify-between items-end gap-4 mb-10 flex-wrap">
              ...
              <Link to="/gallery" className="font-barlow-condensed font-bold text-xs tracking-wider text-chalk light:text-chalk-light px-6 py-2.5 border border-white/40 light:border-black/40 transition-all duration-200 hover:border-accent light:hover:border-accent-light hover:text-accent light:hover:text-accent-light no-underline">
                All Photos →
              </Link>
            </div>
