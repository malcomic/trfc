...
export default function Events() {
  ...
  useEffect(() => {
    if (document.getElementById('trfc-events-styles')) return
  ...
  return (
    <div className="trfc-events">

      {/* ── Hero ── */}
      <section className="trfc-events__hero">
        <div className="trfc-events__hero-line" />
        <div className="trfc-events__hero-bg-num" aria-hidden="true">
          {loading ? '00' : String(events.length).padStart(2, '0')}
        </div>
        <div className="trfc-events__hero-inner">
          <div className="trfc-events__eyebrow">On The Calendar</div>
          <h1 className="trfc-events__title">
            UPCOMING<br /><span>EVENTS</span>
          </h1>
          <div className="trfc-events__hero-meta">
            <div className="trfc-events__hero-stat">
              <span className="trfc-events__hero-stat-val">
                {loading ? '—' : events.length}
              </span>
              <span className="trfc-events__hero-stat-label">Events Scheduled</span>
            </div>
            <div className="trfc-events__hero-sep" />
            <div className="trfc-events__hero-stat">
              <span className="trfc-events__hero-stat-val">NBI</span>
              <span className="trfc-events__hero-stat-label">Nairobi & Beyond</span>
            </div>
            <div className="trfc-events__hero-sep" />
            <div className="trfc-events__hero-stat">
              <span className="trfc-events__hero-stat-val">FREE</span>
              <span className="trfc-events__hero-stat-label">To Register</span>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ── Toolbar ── */}
      <div className="trfc-events__toolbar">
        <div className="trfc-events__toolbar-inner">
          {/* Search */}
          <div className="trfc-events__search-wrap">
            <span className="trfc-events__search-icon"><Search size={14} /></span>
            <input
              className="trfc-events__search"
              type="text"
            ...
            />
            {search && (
              <button className="trfc-events__search-clear" onClick={() => setSearch('')}>
                <X size={13} />
              </button>
            )}
          </div>
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={`trfc-events__filter-btn${activeFilter === f ? ' active' : ''}`}
            >
              {f}
            </button>
          ))}
      ...
      {/* ── Main content ── */}
      <div className="trfc-events__main">

        {/* Error */}
        {error && (
          <div className="trfc-events__error">
        ...
        {/* Skeleton loading */}
        {loading && (
          <div className="trfc-events__skeleton-grid">
            {Array(6).fill(null).map((_, i) => (
              <div key={i} className="trfc-events__skeleton">
                <div className="trfc-events__skeleton-img" />
                <div className="trfc-events__skeleton-body">
                  <div className="trfc-events__skeleton-line" style={{ width: '70%' }} />
                  <div className="trfc-events__skeleton-line" style={{ width: '45%' }} />
                  <div className="trfc-events__skeleton-line" style={{ width: '55%' }} />
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Results label */}
        {!loading && !error && (
          <p className="trfc-events__results-label">
            Showing <span>{filtered.length}</span> of {events.length} event{events.length !== 1 ? 's' : ''}
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="trfc-events__empty">
            <div className="trfc-events__empty-word">NO<br />EVENTS</div>
            <p className="trfc-events__empty-title">
              {search || activeFilter !== 'All' ? 'No matches found' : 'Nothing scheduled yet'}
            </p>
            <p className="trfc-events__empty-sub">
        ...
        {/* Event grid */}
        {!loading && !error && filtered.length > 0 && (
          <div className="trfc-events__grid">
            {filtered.map((event, idx) => {
              ...
              return (
                <Link
                  to={`/events/${event.id}`}
                  key={event.id}
                  className={`trfc-events__card${isFeatured ? ' trfc-events__card--featured' : ''}`}
                >
                  {/* Image */}
                  <div className="trfc-events__card-img-wrap">
                    <img
                      src={(event as any).image_url || 'https://images.unsplash.com/photo-1571008887538-b36bb32f4571?w=800&q=80'}
                      alt={event.title}
                      className="trfc-events__card-img"
                      onError={(e) => {
                        (e.target as HTMLImageElement).src =
                          'https://images.unsplash.com/photo-1571008887538-b36bb32f4571?w=800&q=80'
                      }}
                    />
                    <div className="trfc-events__card-img-gradient" />

                    {/* Date badge */}
                    <div className="trfc-events__card-date-badge">
                      <div className="trfc-events__card-date-day">{day}</div>
                      <div className="trfc-events__card-date-mon">{mon}</div>
                    </div>

                    {/* Category tag */}
                    {(event as any).category && (
                      <span className="trfc-events__card-tag">
                        {(event as any).category}
                      </span>
                    )}
                  </div>
                  ...
                  {/* Body */}
                  <div className="trfc-events__card-body">
                    {/* EventCard renders existing content (description, etc.) */}
                    <EventCard event={event} />

                    <div className="trfc-events__card-footer">
                      <div>
                        <div className="trfc-events__card-price">
                          {(event as any).price === 0 || !(event as any).price
                            ? 'FREE'
                            : `KES ${Number((event as any).price).toLocaleString()}`}
                        </div>
                        <div className="trfc-events__card-price-label">Entry Fee</div>
                      </div>
                      <div className="trfc-events__card-cta">
