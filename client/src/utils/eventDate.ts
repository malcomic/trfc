...
export function formatEventDateTime(value: string | Date | null | undefined): string {
  const d = parseEventDate(value)
  if (!d) return ''
...
export function eventDateParts(value?: string | Date | null) {
  const d = parseEventDate(value)
  if (!d) return { day: null as string | null, mon: null as string | null }
  return {
